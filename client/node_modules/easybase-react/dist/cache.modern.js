...
class Storage {
  ...
  async getBatchDataWithIds(params) {
    ...
    const tasks = ids.map(id => this.load({
      key,
      id,
      syncInBackground,
      autoSync: false,
  ...
  _lookupGlobalItem(params) {
    ...
    return this.getItem(key).then(ret => this._loadGlobalItem(_extends({
      ret
    }, params)));
  }
  ...
  _loadMapItem(params) {
    ...
    if (typeof ret === 'string') {
      ...
      const newId = this._getId(_key, _id);
  ...
  _lookUpInMap(params) {
    ...
    if (m[newId] !== undefined) {
      return this.getItem('map_' + m[newId]).then(ret => this._loadMapItem(_extends({
        ret
      }, params)));
    }
  ...
  remove(params) {
    return this._mapPromise.then(() => {
      ...
      let newId = this._getId(key, id); // remove existing data
  ...
  clearMapForKey(key) {
    return this._mapPromise.then(() => {
      let tasks = (this._m.__keys__[key] || []).map(id => this.remove({
        key,
        id
      }));
...
async function getCacheTokens(cookieName) {
  let cacheToken = false;
  let cacheRefreshToken = false;
  let cacheSession = false;

  try {
    cacheToken = await storage.load({
      key: cookieName + "token"
    });
  } catch (_) {}

  try {
    cacheRefreshToken = await storage.load({
      key: cookieName + "refreshToken"
    });
  } catch (_) {}

  try {
    cacheSession = await storage.load({
...
async function clearCacheTokens(cookieName) {
...
async function setCacheTokens(g, cookieName) {
  await storage.save({
    key: cookieName + "token",
    data: g.token,
...
export { clearCacheTokens, getCacheTokens, setCacheTokens };
//# sourceMappingURL=cache.modern.js.map
